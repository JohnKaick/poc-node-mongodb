...
const {
    ...
    cadastrar,
    editar
} = require('controllers/identity/colaborador')
...
module.exports.obterTodos = async function (request, reply) {
    try {
        const result = await obterTodos(request.auth.credentials.empresaId, request.query)
        reply(result)
    } catch (err) {
        eh.resolve(request, reply, err)
    }
...
module.exports.cadastrar = async function (request, reply) {
    try {
        const result = await cadastrar(request.auth.credentials, request.payload)
        reply(result)
    } catch (err) {
        eh.resolve(request, reply, err)
    }
